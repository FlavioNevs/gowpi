...
pub trait Handler: Send + Sync + 'static {
    async fn invoke(&self, ctx: Context) -> Response;
}
...
pub struct Router {
    map: HashMap<Method, InternalRouter<Box<dyn Handler>>>,
}
...
impl Router {
    ...
    pub async fn get(&mut self, path: &str, handler: Box<dyn Handler>) {
        self.map
            .entry(Method::GET)
    ...
    pub async fn post(&mut self, path: &str, handler: Box<dyn Handler>) {
        self.map
            .entry(Method::POST)
            .or_insert_with(InternalRouter::new)
            .add(path, handler)
